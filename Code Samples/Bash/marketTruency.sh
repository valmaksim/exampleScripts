...
do
        ...
        curl -s -A "valmaksim.gromakov@gmail.com" "https://api.eveonline.com/char/WalletTransactions.xml.aspx?characterID=${charID}&rowCount=100&keyID=${keyID}&vCode=${vCode}" | sed -n 's/.*transactionID=\"\([0-9]*\)\".*quantity=\"\([0-9]*\)\".*typeID=\"\([0-9]*\)\".*transactionType="sell".*/\3 \2 \1/p' > sellInfo
...
if [ ! -e report ]
    then
        echo "No Infractions This Week" | mail -s "Market Alerts for Week Ending `date +%m-%d-%Y`" "valmaksim.gromakov@gmail.com"
        exit 0
    else
        cat report > ~corpscripts/marketMonitoring/infractions/W.E.`date +%m-%d-%Y`.report
        cat report | mail -s "Market Alerts for Week Ending `date +%m-%d-%Y`" "valmaksim.gromakov@gmail.com"
fi
