...
curl -s -A "email@domain.com" "https://api.eveonline.com/corp/Contracts.xml.aspx?keyID=5406883&vCode=yQqASdKh8maitFFG0eV7msO4QNJ6aBLadsu8pgkDBCZSFgGjqo132bBAtywSZBdd" | sed -n 's/.*contractID=\"\([0-9]*\)\".*issuerID=\"\([0-9]*\)\".*type=\"ItemExchange\".*status=\"Completed\".*price=\"0\.00\".*/\1 \2/p' > contractID_issuerID
    ...
    then
        if [ -z "$charName" ]
            then
                ...
                curl -s -A "email@domain.com" "https://api.eveonline.com/corp/ContractItems.xml.aspx?contractID=${getContractID}&keyID=5406883&vCode=yQqASdKh8maitFFG0eV7msO4QNJ6aBLadsu8pgkDBCZSFgGjqo132bBAtywSZBdd" | sed -n 's/.*typeID=\"\([0-9]*\)\".*quantity=\"\([0-9]*\)\".*/\1 \2/p' > tempContractInfo
                        ...
                        if [ "$getWHLootID" = "$typeID" ]
                           then
                                if [ "$typeID" = "30744" -o "$typeID" = "30745" -o "$typeID" = "30746" -o "$typeID" = "30747" ]
                                    then
                                        price=`curl -s -A "email@domain.com" "http://api.eve-central.com/api/marketstat?typeid=${typeID}&usesystem=30000163" | sed -n 's/.*<buy>.*<max>\([0-9]*.[0-9][0-9]\).*/\1/p'`
                                        price=`calc ${price}*${quantity}`
                                        price=`calc ${price}*0.85`
                                        price=`echo ${price}`
                                        printf "${price} " >> totals
                                    else
                                        price=`curl -s -A "email@domain.com" "http://api.eve-central.com/api/marketstat?typeid=${typeID}&usesystem=30000142" | sed -n 's/.*<buy>.*<max>\([0-9]*.[0-9][0-9]\).*/\1/p'`
                                        price=`calc ${price}*${quantity}`
                                        price=`calc ${price}*0.85`
                                        price=`echo ${price}`
                                        printf "${price} " >> totals
...
if [ ! -e totals ]
    then
        echo "No Payout Reports This Week" | mail -s "Income Report for Payperiod Ending on: `date +%m-%d-%Y`" "email@domain.com"
        exit 0
    else
        cat totals > ~corpscripts/buybackProgram/incomeReports/`date +%m-%d-%Y`.report
        cat totals | mail -s "Income Report for Payperiod Ending on: `date +%m-%d-%Y`" "email@domain.com"
fi
