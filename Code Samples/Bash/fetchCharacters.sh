...
do
        ...
        apiAuth=`curl -s -A "valmaksim.gromakov@gmail.com" "https://api.eveonline.com/account/APIKeyInfo.xml.aspx?keyID=${keyID}&vCode=${vCode}" | sed -n 's/.*accessMask=\"\(.*\)\".*type=\"\(.*\)\".*expires=\"\(.*\)\".*/\1 \2 \3/p'`
...
if [ ! -e keyInfraction.tmp ]
    then
        ...
        cat keyInfraction.tmp | mail -s "Improper Keys Were Found" "valmaksim.gromakov@gmail.com"
